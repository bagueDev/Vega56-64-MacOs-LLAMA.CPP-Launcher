...
DEFAULT_MODELS = "/Volumes/CrucialX8/AI_Models"
LAUNCHER_PORT = 9999
SERVER_PORT = 8080
# ──────────────────────────────────────────────────────────────
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .no-models{text-align:center;padding:20px;color:var(--muted);font-size:0.8rem;}
   ...
   .btn-preset{flex:1;background:var(--bg);border:1px solid var(--border);border-radius:6px;
                padding:10px 8px;font-family:var(--mono);font-size:0.72rem;color:var(--muted);
                cursor:pointer;text-align:center;transition:all .2s;letter-spacing:.03em;}
   .btn-preset:hover{border-color:var(--muted);color:var(--text);}
   .btn-preset.active{border-color:var(--accent);color:var(--accent);background:rgba(255,107,53,.06);}
    .btn-preset .preset-icon{font-size:1.1rem;display:block;margin-bottom:4px;}
   .toggle-row{display:flex;gap:16px;grid-column:1/-1;margin-top:4px;}
   .toggle-label{display:flex;align-items:center;gap:6px;font-size:0.72rem;color:var(--muted);cursor:pointer;user-select:none;}
...
<div class="card">
  ...
  <div class="preset-row">
    <button class="btn-preset" onclick="applyPreset('code')"><span class="preset-icon">&lt;/&gt;</span>Code</button>
    <button class="btn-preset active" onclick="applyPreset('chat')"><span class="preset-icon">&#9993;</span>Chat</button>
    <button class="btn-preset" onclick="applyPreset('kreativ')"><span class="preset-icon">&#9733;</span>Kreativ</button>
  </div>
  <div class="options">
    <div class="opt-group">
      <div class="opt-label">Kontext</div>
      <input class="opt-input" id="ctxSize" value="16384" type="number" step="512">
    </div>
    <div class="opt-group">
    ...
    <div class="opt-group">
      <div class="opt-label">KV Cache</div>
      <select class="opt-select" id="kvCache">
        <option value="q4_0">q4_0 — Klein, spart VRAM</option>
        <option value="q8_0">q8_0 — Balanced</option>
        <option value="f16" selected>f16 — Präzise (empfohlen)</option>
        <option value="f32">f32 — Volle Genauigkeit</option>
      </select>
    </div>
    <div class="opt-group">
      <div class="opt-label">Max Tokens</div>
      <input class="opt-input" id="maxTokens" value="2048" type="number" step="256" min="1">
    </div>
    <div class="opt-group">
      <div class="opt-label">Temperature</div>
      <input class="opt-input" id="temperature" value="0.7" type="number" step="0.1" min="0" max="2">
    </div>
    <div class="opt-group">
      <div class="opt-label">Top K</div>
      <input class="opt-input" id="topK" value="40" type="number" step="1" min="1" max="200">
    </div>
    <div class="opt-group">
      <div class="opt-label">Repeat Penalty</div>
      <input class="opt-input" id="repeatPenalty" value="1.0" type="number" step="0.05" min="1" max="2">
    </div>
    <div class="opt-group">
      <div class="opt-label">Batch Size (-b)</div>
      <input class="opt-input" id="batchSize" value="512" type="number" step="64" min="64">
    </div>
    <div class="opt-group">
      <div class="opt-label">uBatch (-ub)</div>
      <input class="opt-input" id="ubatchSize" value="512" type="number" step="64" min="64">
    </div>
    <div class="opt-group" style="grid-column:1/-1">
      <div class="opt-label">Extra Flags</div>
      <input class="opt-input" id="extraFlags" placeholder="z.B. --slot --flash-attn --verbose" style="font-size:0.7rem">
    </div>
    <div class="toggle-row">
      <label class="toggle-label"><input type="checkbox" id="optJinja" checked> --jinja</label>
      <label class="toggle-label"><input type="checkbox" id="optMcp" checked> --webui-mcp-proxy</label>
      <label class="toggle-label"><input type="checkbox" id="optNoKvOffload"> --no-kv-offload</label>
    </div>
  </div>
...
function updatePreview() {
  ...
  const kv = document.getElementById('kvCache').value;
  const mt = document.getElementById('maxTokens').value;
  const ef = document.getElementById('extraFlags').value.trim();
  const temp = document.getElementById('temperature').value;
  const topk = document.getElementById('topK').value;
  const rp = document.getElementById('repeatPenalty').value;
  const batchSz = document.getElementById('batchSize').value;
  const ubatchSz = document.getElementById('ubatchSize').value;
  const noKvOff = document.getElementById('optNoKvOffload').checked;
  const jinja = document.getElementById('optJinja').checked;
  const mcp = document.getElementById('optMcp').checked;
  let cmd = '<span>llama-server</span> -m ' + selectedModel.name +
    ' -ngl ' + ngl + ' --device Vulkan0' +
    ' --ctx-size ' + ctx + ' --threads ' + threads + ' --port ' + port +
    ' --cache-type-k ' + kv + ' --cache-type-v ' + kv +
    ' -n ' + mt +
    ' --batch-size ' + batchSz + ' --ubatch-size ' + ubatchSz;
  if (jinja) cmd += ' --jinja';
  if (mcp) cmd += ' --webui-mcp-proxy';
  if (noKvOff) cmd += ' --no-kv-offload';
  if (tpl !== 'auto') cmd += ' --chat-template ' + tpl;
  cmd += ' <span style="color:var(--muted)"># sampling: temp=' + temp + ' top_k=' + topk + ' repeat_penalty=' + rp + '</span>';
  if (ef) cmd += ' ' + ef;
...
function applyPreset(name) {
  const presets = {
    code:    { temperature: 0.2, topK: 30,  repeatPenalty: 1.15, maxTokens: 4096 },
    chat:    { temperature: 0.7, topK: 40,  repeatPenalty: 1.0,  maxTokens: 2048 },
    kreativ: { temperature: 1.1, topK: 60,  repeatPenalty: 1.0,  maxTokens: 2048 }
  };
  const p = presets[name]; if (!p) return;
  document.getElementById('temperature').value = p.temperature;
  document.getElementById('topK').value = p.topK;
  document.getElementById('repeatPenalty').value = p.repeatPenalty;
...
['ctxSize','ngl','port','threads','chatTemplate','kvCache','maxTokens','extraFlags','temperature','topK','repeatPenalty','batchSize','ubatchSize'].forEach(id =>
  document.getElementById(id).addEventListener('input', updatePreview)
);
['optJinja','optMcp','optNoKvOffload'].forEach(id =>
...
async function startServer() {
  ...
  const body = {
    ...
    cache_type: document.getElementById('kvCache').value,
    max_tokens: parseInt(document.getElementById('maxTokens').value) || 2048,
    extra_flags: document.getElementById('extraFlags').value.trim(),
    temperature: parseFloat(document.getElementById('temperature').value) || 0.7,
    top_k: parseInt(document.getElementById('topK').value) || 40,
    repeat_penalty: parseFloat(document.getElementById('repeatPenalty').value) || 1.0,
    jinja: document.getElementById('optJinja').checked,
    mcp_proxy: document.getElementById('optMcp').checked,
    batch_size: parseInt(document.getElementById('batchSize').value) || 512,
    ubatch_size: parseInt(document.getElementById('ubatchSize').value) || 512,
    no_kv_offload: document.getElementById('optNoKvOffload').checked,
  };
  try {
...
function openChat() {
  ...
  const topk = document.getElementById('topK').value;
  const rp = document.getElementById('repeatPenalty').value;
  window.open('/chat?port=' + port + '&ctx=' + ctx + '&max_tokens=' + mt +
    '&temperature=' + temp + '&top_k=' + topk + '&repeat_penalty=' + rp, '_blank');
}
...
<script>
  ...
  const SERVER_MAX_TOKENS = parseInt(params.get('max_tokens')) || 2048;
  const SERVER_TEMPERATURE = parseFloat(params.get('temperature')) || 0.7;
  const SERVER_TOP_K = parseInt(params.get('top_k')) || 40;
  const SERVER_REPEAT_PENALTY = parseFloat(params.get('repeat_penalty')) || 1.0;
  ...
  async function sendMessage() {
    ...
    try {
      const r = await fetch(SERVER_URL + '/v1/chat/completions', {
        method: 'POST', headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
          model: 'local', messages: apiMsgs, stream: true,
          max_tokens: SERVER_MAX_TOKENS,
          temperature: SERVER_TEMPERATURE,
          top_k: SERVER_TOP_K,
          repeat_penalty: SERVER_REPEAT_PENALTY
        })
      });
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            log_line = ""
            if running and proc.stderr:
                try:
                    import select

                    if select.select([proc.stderr], [], [], 0)[0]:
                        log_line = (
                            proc.stderr.readline().decode(errors="replace").strip()
                        )
                        if log_line:
            ...
            self.send_json(
                {
                    "running": running,
                    "port": port,
                    "model": Path(dirs[0]).name if running and dirs else "",
                    "log_line": log_line,
                }
            )
    ...
    def do_POST(self):
        ...
        if self.path == "/api/start":
            with server_lock:
                ...
                cache_type = body.get("cache_type", "f16")
                max_tokens = body.get("max_tokens", 2048)
                extra_flags = body.get("extra_flags", "").strip()
                # Gemma 4 requires f16 KV cache - override user selection
                model_name = Path(body.get("model", "")).stem.lower()
            ...
            cmd = [
                ...
                str(max_tokens),
                "--batch-size",
                str(body.get("batch_size", 512)),
                "--ubatch-size",
                str(body.get("ubatch_size", 512)),
                "--no-warmup",
            ]
            # Optional flags from UI toggles
            if body.get("jinja", True):
                cmd.append("--jinja")
            if body.get("mcp_proxy", True):
                cmd.append("--webui-mcp-proxy")
            if body.get("no_kv_offload", False):
                cmd.append("--no-kv-offload")
            # Only add --chat-template if not auto (auto = let llama.cpp read from GGUF)
            if tpl and tpl != "auto":
                cmd += ["--chat-template", tpl]
            # Add extra flags (all flags allowed for testing)
            if extra_flags:
            ...
            try:
                ...
                with server_lock:
                    egpu_process = new_proc
                self.send_json({"ok": True, "pid": new_proc.pid})
            except Exception as e:
            ...
            with server_lock:
                ...
                dirs = list(models_dirs)
            self.send_json({"ok": True, "dirs": dirs})
